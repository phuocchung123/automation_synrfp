                    ...
                    for file in files:
                        ...
                        for i in range(df_train.shape[0]):
                            try:
                                x_train_lst.append(fps(df_train.loc[i,'rxn'],tokenizer=tokenizer,radius=radius,sketch=sketch,mode=mode,bits=bits))
                                        ...
                                        # In rõ lỗi gì và ép ghi vào file ngay lập tức
                                        logger.info(f'Failed at index {i} in {file}. Reason: {e}')
                                        
                                        # Ép log ghi xuống ổ cứng phòng khi crash
                        ...
                        for i in range(df_test.shape[0]):
                            try:
                                ...
                                logger.info(f'failed idx test is {i} in {file} dataset')
...
if __name__ == "__main__":
    ...
    main('knn', folder_data='data/raw_trial')
    # main('mlp')
    # main('xgb')
